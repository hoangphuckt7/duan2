...
class MY_Model extends CI_Model
{
	...
	/**
	 * Them slug moi
	 * $data : du lieu ma ta can tao
	 */
	public function get_slug($slug = FALSE)
	{
        if ($slug === FALSE)
        {
                $query = $this->db->get('url_slug');
                return $query->result_array();
        }

        $query = $this->db->get_where('url_slug', array('slug' => $slug));
	...
	function getNews_VN($limit, $start) {
		$this->db->like('code', 'VN'); 
        $this->db->order_by('id desc');
        $query = $this->db->get($this->table, $limit, $start);
        return $query->result();
    }
    function get_total_rows_VN()
    {
    	$this->db->from($this->table);
    	$count = $this->db->like('code','VN')->count_all_results();
    	return $count;
    }
    
    function getNews_NB($limit, $start) {
		$this->db->like('code', 'NB'); 
        $this->db->order_by('id desc');
        $query = $this->db->get($this->table, $limit, $start);
        return $query->result();
    }
    function get_total_rows_NB()
    {
    	$this->db->from($this->table);
    	$count = $this->db->like('code','NB')->count_all_results();
    	return $count;
    }

    function get_list_vietseiko() {
		$sql = "select * from news where code ='VIETSEIKO'";
		$query = $this->db->query($sql);
        return $query->result();
    }
    function get_total_vietseiko() {
		$sql = "select * from news where code ='VIETSEIKO'";
		$query = $this->db->query($sql);
        return $query->num_rows();
    }
